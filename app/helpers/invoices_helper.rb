module InvoicesHelper
  ...
  def render_invoice(invoice)
    ...
    pdf.font_families.update(
      'OpenSans' => { :normal => Rails.root.join('fonts/OpenSans-Regular.ttf').to_s,
    ...
    pdf.table items, :header => true, 
      :column_widths => {0 => 350}, :row_colors => ["d2e3ed", "FFFFFF"] do
  ...
  def invoice_items(invoice)
    ...
    invoice.project.labels.each do |l|
      ...
      items << ["#{l.name}", "#{l.wage.to_s} #{invoice.project.currency}", "#{time.round(2)}", "#{(time * l.wage).round(2)} #{invoice.project.currency}"] if time > 0
    end
    items
  end
end
